...
class Page():
    ...
    def js_view(self, script, css):
        """
        Execute JavaScript scripts.

        Usage:
        driver.js_view("arguments[0].scrollIntoView();", ele)
        """
        t1 = time.time()
        css1 = css[0] + "->" + css[1]
        try:
    ...
    # 可以获取元素属性（标签）,get_attribute(self,'type') --> 返回属性值
    # 可以获取文本框输入的值 get_attribute(self,'value') --> 返回输入框输入的内容
    ...
    # 可以获取元素属性（标签）,get_property(self,'type') --> 返回属性值
    # 可以获取文本框输入的值 get_property(self,'value') --> 返回输入框输入的内容
    def get_property(self, css, value):
        """
        Gets the value of an element attribute.
        ...
        try:
            el = self.get_element(css)
            attr = el.get_property(value)
            self.my_print("{0} Get attribute element: <{1}>,attribute: {2}, Spend {3} seconds".format(success,
                css,value,time.time()-t1))
            return attr
        except Exception:
            self.my_print("{0} Unable to get attribute element: <{1}>,attribute: {2}, Spend {3} seconds".format(fail,
                css, value,time.time() - t1))

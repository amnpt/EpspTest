...
# 通过python代码去调用tesseract以达到识别验证码的效果
...
TESSERACT = 'F:/Tesseract-OCR/tesseract.exe'  # 调用的本地命令名称
...
def perform_cleanup(TEMP_IMAGE_NAME, TEMP_RESULT_NAME):
    ...
    for name in (TEMP_IMAGE_NAME, TEMP_RESULT_NAME + '.txt'):
...
def image_to_string(image, lang, cleanup=CLEANUP_TEMP_FLAG, incompatible=INCOMPATIBLE):
    ...
    # logging.basicConfig(filename='G:/8.python/pytest_testApi/logs/tesseract.log')
    # logging.debug('image_to_string')
    logging.basicConfig(filename='tesseract.log')
    try:
        try:
            call_tesseract(image, TEMP_RESULT_NAME, lang='eng')
